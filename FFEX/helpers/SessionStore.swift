...
final class SessionStore: NSObject, ObservableObject {
    ...
    private func startRevalLoop() {
        ...
        // Observe foreground notification — revalidate instantly when app comes to front
    ...
    @objc private func handleForegroundNotification() {
        guard let key = licenseInfo?.key else { return }
        Task { await revalidateOnce(key: key) }
    }
