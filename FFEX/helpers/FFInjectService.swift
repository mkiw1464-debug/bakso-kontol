...
enum FFInjectService {
    ...
    private static func stampLocalConfig(data: Data, session: InjectSession) -> Data {
        // Use CheatSettings to build config — this is what the game patch reads
        let cheat = CheatSettings.shared
        if let configData = cheat.toJSON(token: session.sessionToken, hwid: DeviceID.hwid) {
            return configData
        }
        // Fallback: stamp existing JSON
    ...
    static func writeFeatureFiles(game: FFGame, session: InjectSession) {
        ...
        let handle = ContainerStore.grantContainerAccess(containerPath)
        defer { if handle >= 0 { bad_query_release(handle) } }
        let docsPath = URL(fileURLWithPath: containerPath)
            .appendingPathComponent("Documents").path
        CheatSettings.shared.writeFeatureFiles(
            to: docsPath,
            token: session.sessionToken,
            hwid: DeviceID.hwid
        )
        for name in CheatSettings.allFeatureFiles {
            let path = (docsPath as NSString).appendingPathComponent(name)
            if FileManager.default.fileExists(atPath: path) {
                session.registerDeployedPath(path)
            }
        }
    }
